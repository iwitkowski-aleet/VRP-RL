...
class Attention(object):
    ...
    def __init__(self, dim, use_tanh=False, C=10,_name='Attention',_scope=''):
        ...
        self.project_query = tf.keras.layers.Dense(dim,name=_scope.replace('/', '_').replace(':', '_')+_name.replace('/', '_') +'_dense')
        self.project_ref = tf.keras.layers.Conv1D(dim,1,name=_scope.replace('/', '_').replace(':', '_')+_name.replace('/', '_') +'_conv1d')
