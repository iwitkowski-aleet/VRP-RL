...
class RLAgent(object):
    ...
    def build_model(self, decode_type = "greedy"):
        ...
        # decoder_state - initialize for Keras 3 LSTM layers
        decoder_state = []
        for i in range(args['rnn_layers']):
            decoder_state.append([tf.zeros([batch_size*beam_width, args['hidden_dim']]), 
                                tf.zeros([batch_size*beam_width, args['hidden_dim']])])            
        ...
        if decode_type=='stochastic':
                with tf.compat.v1.variable_scope("Critic"):
                    with tf.compat.v1.variable_scope("Encoder"):
                        # init states for Keras 3 LSTM layers
                        rnn_tuple_state = []
                        for i in range(args['rnn_layers']):
                            rnn_tuple_state.append([tf.zeros([batch_size, args['hidden_dim']]), 
                                                  tf.zeros([batch_size, args['hidden_dim']])])

                    hy = rnn_tuple_state[0][0]  # Get hidden state from first layer
                ...
                with tf.compat.v1.variable_scope("Linear"):
                    # Create layers within the variable scope
                    l1 = tf.keras.layers.Dense(args['hidden_dim'], activation=tf.nn.relu, name='Critic_L1')
                    l2 = tf.keras.layers.Dense(1, name='Critic_L2')
                    v = tf.squeeze(l2(l1(hy)), 1)
    ...
    def build_train_step(self):
        ...
        critic_loss = tf.keras.losses.MeanSquaredError()(R, v)
        ...
        # Check if critic variables exist
        critic_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='Critic')
        if critic_vars:
            critic_gra_and_var = critic_optim.compute_gradients(critic_loss, critic_vars)
        else:
            # If no critic variables, create empty gradients
            critic_gra_and_var = []
        ...
        if critic_gra_and_var:
            ...
            critic_train_step = tf.no_op()  # No-op if no critic variables
    ...
    def load_model(self):
        latest_ckpt = tf.compat.v1.train.latest_checkpoint(self.args['load_path'])
